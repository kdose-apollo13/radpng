    ...
    for vname, vcall in variants.items():
        for sz in sizes:
            ...
            for lab, m in zip(measurer_labels, measurers):
                dt = time_call(vcall, *a, **k, measurer=m)
                times[lab] = dt
...
if __name__ == '__main__':
    ...
    # with arg_builder and (w,h) size
    def build(n):
        return (int(n),), {}

    rows2 = scale(_demo_work, [8, 16], arg_builder=build, warmup=1)
    assert rows2[0]['pixels'] is None  # not (w,h) tuple
